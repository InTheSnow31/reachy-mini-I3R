...
from emotion import emotion, polar_to_emotion, random_emotion
...
class SoundGenEnv(gym.Env):
    def __init__(self, emotion_model, max_notes=16):
        ...
        self.num_intensity_bins = 10  # Discrétisation de l'intens
        self.duration_range = 4  # Durée en temps discrets

        self.action_space = spaces.MultiDiscrete([self.note_range, self.duration_range, self.num_intensity_bins, 2, 2]) # Notes 44-67, Durée 1-4, Intensité 0-(num_intensity_bins-1), Slide ?, Stop ?
    ...
    def reset(self):
        ...
        # Emotion cible
        self.target_emotion = random_emotion(self.EMOTION_MODEL).l()
    ...
    def _get_obs(self):
        ...
        # Émotion cible normalisée
        obs_emotion = self.target_emotion
        return np.concatenate([obs_notes, obs_emotion])  # Box 1D
    ...
    def evaluate_sequence(self, notes):
        emotion_generated = self.estimate_emotion(notes).l()  # vecteur de taille 5
        # Reward = négatif de la distance Euclidienne
