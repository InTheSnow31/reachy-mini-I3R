...
# -------------------------------
# Modèle simple LSTM pour prétraining
# -------------------------------
class PretrainPolicy(nn.Module):
    def __init__(self, obs_dim, hidden_dim=128, num_layers=1):
        with Path("sound_config.json").open("r", encoding="utf-8") as f:
            ...
            action_dims=[json_content["TONES_RANGE"], json_content["DURATION_SCALE"], json_content["MAX_NOTES"], 2, 2]
        super().__init__()
        self.lstm = nn.LSTM(input_size=obs_dim, hidden_size=hidden_dim, num_layers=num_layers, batch_first=True)
...
# -------------------------------
# Fonction de pré-entraînement avec validation
# -------------------------------
def pretrain_with_val(dataset_folder, policy, epochs=100, batch_size=8, lr=1e-3, max_notes=16, num_emotions=3, val_ratio=0.2, device='cpu'):
    # Chargement dataset
    ...
    ce_losses = [nn.CrossEntropyLoss() for _ in range(5)]  # 5 dimensions MultiDiscrete
    ...
    for epoch in range(epochs):
        ...
        for emotions, actions, lengths in train_loader:
            ...
            for b in range(batch_size_):
                hist = torch.zeros(max_notes, 4)
                seq_obs = []
                for t in range(seq_len):
                    start = max(0, t - max_notes)
                    hist[-min(t,max_notes):] = actions[b, start:t, :4].float() if t>0 else 0
                    seq_obs.append(torch.cat([hist.flatten(), emotions[b]]))
...
def plot_losses(train_losses, val_losses):
    ...
    train_losses, val_losses: listes de float, longueur = nb d'epochs
    """
